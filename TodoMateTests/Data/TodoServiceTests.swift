...
fileprivate struct TestFixtures {
    ...
    static let mockTodo = Todo(date: now, content: "test", detail: "test-detail", status: .todo, uid: "test-uid", fid: "test-fid", lastModifiedAt: now)
    static let mockTodoDTO = mockTodo.toDTO()
    static let mockTodos = [mockTodo]
    static let mockTodoDTOs = [mockTodoDTO]
    
    // create 성공
    static func todoRepositoryForCreateSuccess(returnValue: TodoDTO = mockTodoDTO) -> MockTodoRepository {
    ...
    // create 실패
    static func todoRepositoryForCreateFailure(error: Error = NSError(domain: "Test", code: -1, userInfo: [NSLocalizedDescriptionKey: "Mock Failure"])) -> MockTodoRepository {
        var repo = MockTodoRepository()
    ...
    // fetchMonth 성공
    static func todoRepositoryForFetchUserMonthSuccess(returnValue: [TodoDTO] = mockTodoDTOs) -> MockTodoRepository {
        var repo = MockTodoRepository()
    ...
    // fetchMonth 실패
    static func todoRepositoryForFetchUserMonthFailure(error: Error = NSError(domain: "Test", code: -1, userInfo: [NSLocalizedDescriptionKey: "Mock Failure"])) -> MockTodoRepository {
        var repo = MockTodoRepository()
    ...
    // fetch(groupId:) 성공
    static func todoRepositoryForFetchGroupSuccess(returnValue: [TodoDTO] = mockTodoDTOs) -> MockTodoRepository {
        var repo = MockTodoRepository()
    ...
    // fetch(groupId:) 실패
    static func todoRepositoryForFetchGroupFailure(error: Error = NSError(domain: "Test", code: -1, userInfo: [NSLocalizedDescriptionKey: "Mock Failure"])) -> MockTodoRepository {
        var repo = MockTodoRepository()
...
struct TodoServiceTests {
    ...
    struct FetchMonthTests {
        ...
        func fetchMonthSucceeds() async {
            // Given
            let todoService = TodoService(todoRepository: TestFixtures.todoRepositoryForFetchUserMonthSuccess())
            let startDate = Date()
            let endDate = Date()
            
            // When
            let result = await todoService.fetchMonth(userId: "test-uid", startDate: startDate, endDate: endDate)
        ...
        func fetchMonthFails() async {
            // Given
            let todoService = TodoService(todoRepository: TestFixtures.todoRepositoryForFetchUserMonthFailure())
            let startDate = Date()
            let endDate = Date()
    ...
    struct FetchTodayGroupTests {
        ...
        func fetchTodaySucceeds() async {
            // Given
            let todoService = TodoService(todoRepository: TestFixtures.todoRepositoryForFetchGroupSuccess())
        ...
        func fetchTodayFails() async {
            // Given
            let todoService = TodoService(todoRepository: TestFixtures.todoRepositoryForFetchGroupFailure())
