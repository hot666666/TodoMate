  ...
  func linkWithCredential(_ credential: AuthCredential) async throws {
    ...
    do {
      ...
      do {
        ...
        if let email = result.user.email, !email.isEmpty {
          update["email"] = email
        } else {
          update["email"] = ""
        }
        if let displayName = result.user.displayName, !displayName.isEmpty {
          update["displayName"] = displayName
        } else {
          update["displayName"] = "User"
        }
        try await db.collection("users").document(result.user.uid).updateData(update)
        ...
        do {
          _ = try await result.user.unlink(fromProvider: credential.provider)
        } catch {
