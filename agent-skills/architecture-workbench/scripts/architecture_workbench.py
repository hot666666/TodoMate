    ...
    for line in lines:
        ...
        if line.startswith("## "):
            if section == "Nodes":
                ...
                document.trace_ids.append(trace_id)
            continue
...
def validate_documents(root: Path, documents: list[Document]) -> dict[str, Any]:
    ...
    for document in documents:
        ...
        for trace_id in document.trace_ids:
            if not STABLE_ID.fullmatch(trace_id):
...
def validate_feedback(root: Path, documents: list[Document], feedback_items: list[Feedback]) -> dict[str, Any]:
    ...
    for item in feedback_items:
        ...
        for index, response in enumerate(item.responses, start=1):
            ...
            if disposition == "needs-review" and response.get("unresolvedReason") in {"", "-", None}:
