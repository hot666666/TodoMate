...
description: End-to-end GitHub issue fix workflow using gh, local code changes, builds/tests, and git push. Use when asked to take an issue number, inspect the issue via gh, implement a fix, run just builds/tests, commit with a closing message, and push.
---
...
3. Follow repo-specific conventions (AGENTS/GEMINI instructions).
...
1. Use just for required builds/tests:
   - Build: `just build`
   - Tests: `just test` (consult the `justfile` for details as there may be multiple test targets).
2. If macOS tests fail due to deployment target mismatches, run the equivalent iOS simulator tests.
3. If tests fail, inspect the logs to identify the root cause.
