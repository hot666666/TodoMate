...
extension CalendarView {
  ...
  private func updateSelection(at index: Int) {
    ...
    if let previousIndex = selectedIndex {
      calendarDays[previousIndex].dateType = calendar
        .isDateInToday(calendarDays[previousIndex].date) ? .today : .default
    }
...
extension CalendarView {
  ...
  private func updateCalendarDays(todoDate: Date) {
    let startOfMonth = calendar.date(from: calendar.dateComponents(
      [.year, .month],
      from: currentDate
    ))!
    let prefixOfFirstWeek = -calendar.component(.weekday, from: startOfMonth) + 1
    let startDayInPreviousMonth = calendar.date(
      byAdding: .day,
      value: prefixOfFirstWeek,
      to: startOfMonth
    )!
