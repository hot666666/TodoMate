...
protocol SyncTodayTodosUseCase {
  func run(for userId: String, in date: Date) async throws
}

final class SyncTodayTodosUseCaseImpl: SyncTodayTodosUseCase {
  private let localRepository: TodoRepository
  private let remoteRepository: TodoRepository

  init(localRepository: TodoRepository, remoteRepository: TodoRepository) {
  ...
  func run(for userId: String, in date: Date) async throws {
    ...
    async let localTodos = try localRepository.readAll(query: query, source: .cache)
    async let remoteTodos = try remoteRepository.readAll(query: query, source: .server)
