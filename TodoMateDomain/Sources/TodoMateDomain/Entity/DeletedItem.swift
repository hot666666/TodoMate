...
public enum DeletedItem: Identifiable, Sendable, Equatable {
  ...
  public var id: String {
    switch self {
    case let .todo(todo): todo.id
    case let .memo(memo): memo.id
    }
  }
  ...
  public var deletedAt: Date {
    switch self {
    case let .todo(todo): todo.updatedAt
    case let .memo(memo): memo.updatedAt
    }
  }
  ...
  public var displayContent: String {
    switch self {
    case let .todo(todo): todo.contentOrPlaceholder
    case let .memo(memo): memo.content.isEmpty ? "빈 메모" : String(memo.content.prefix(50))
    }
  }
